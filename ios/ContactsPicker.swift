...
protocol ContactPickerDelegateDelegate: AnyObject {
  func done()
}
...
class PickContactDelegate: NSObject, CNContactPickerDelegate {
  ...
  func contactPicker(_ picker: CNContactPickerViewController, didSelect contact: CNContact) {
    resolve?(ContactsPicker.contactToDictionary(contact))
    clear()
    delegate.done()
  }
}
...
class PickContactsDelegate: NSObject, CNContactPickerDelegate {
  ...
  func contactPicker(_ picker: CNContactPickerViewController, didSelect contacts: [CNContact]) {
    resolve?(contacts.map { x in ContactsPicker.contactToDictionary(x) })
    clear()
    delegate.done()
  }
}
...
@objc(ContactsPicker)
class ContactsPicker: NSObject, ContactPickerDelegateDelegate {
  ...
  func present(viewController: UIViewController) {
    ...
    DispatchQueue.main.async { [weak self] in
      self?.getTopViewController(window: keyWindow)?.present(viewController, animated: true, completion: nil)
    }
  }
  ...
  static func addLabel<T>(_ data: inout [String: Any], item: CNLabeledValue<T>) {
    ...
    if let label = item.label {
      if label.hasPrefix("_$!<") && label.hasSuffix(">!$_") {
        addString(data: &data, key: "label", value: String(label[label.index(label.startIndex, offsetBy: 4)..<label.index(label.endIndex, offsetBy: -4)]))
      } else {
