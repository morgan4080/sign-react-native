            ...
            <SafeAreaView style={{ flex: 1, backgroundColor: '#ffffff', borderTopLeftRadius: 25, borderTopRightRadius: 25 }}>
                <ScrollView contentContainerStyle={{display: 'flex', alignItems: 'center', justifyContent: 'center', width, height: height/1.5}}>
                    <View style={{display: 'flex', alignItems: 'center'}}>
                        <Text allowFontScaling={false} style={{ fontFamily: 'Poppins_700Bold', color: '#323492', fontSize: 20, maxWidth: 250, textAlign: 'center', marginTop: (height/1.5)/30 }}>Witness Request {route.params?.accepted ? 'Accepted' : 'Declined'}</Text>
                        ...
                        <Text allowFontScaling={false} style={{ fontFamily: 'Poppins_400Regular', color: '#9a9a9a', fontSize: 12, maxWidth: 250, textAlign: 'center', marginTop: (height/1.5)/20 }}>
                            You have {route.params?.accepted ? 'agreed' : 'declined'} to take on Witness responsibility for <Text style={{color: '#323492'}}>{request?.executor}</Text> for a loan amount <Text style={{color: '#323492'}}>{request?.subject}</Text>
                        </Text>
                    </View>
                </ScrollView>
            </SafeAreaView>
